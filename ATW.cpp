...
void Graph::buildGraph(){
	...
	addEdge("Delhi","Yokohama", 3900, 19, "with a stop in Beijing then a ship to Japan. Follows along the Silk Road");  //Stop in Beijing(2300 miles) and 1600 miles to yokohama by boat
	...
	addEdge("Yokohama", "San Francisco", 5200, 20, "via a steamship. A long journey, follows strong ocean currents."); //20 days
	...
	addEdge("Chicago", "New York", 800, 2, "on a scenic coal trail through the northeast."); //rail
...
void Graph::printNextCity(string currCity){
	for(int i = 0; i < vertices.size(); i++){
...
void Graph::travelCity(string prevCity, string nextCity){
	for(int i = 0; i < vertices.size(); i++){
        if(vertices[i].cityName == prevCity){			//Goes to currCity vertex
        	for(int j = 0; j < vertices[i].adj.size(); j++){
        		if(vertices[i].adj[j].v->cityName == nextCity){
        			currCity = vertices[i].adj[j].v->cityName;
        			day = day + vertices[i].adj[j].travelTime;
        			prevTravel = vertices[i].adj[j].travelTime;
        		}
        	}
        }
    }
